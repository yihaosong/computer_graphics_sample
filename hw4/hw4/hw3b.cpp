...
enum {MV, PROJ, SAMPLER, DRAW_TEXTURE};
enum {WIREFRAME, TEXTURED, TEXTURED_WIREFRAME};
enum {FLAT_SHADING, SMOOTH_SHADING, SMOOTH_TEXTURE_SHADING};
...
HW3b::controlPanel()
{
	...
	// create Shading comboBox
	m_comboBoxShading = new QComboBox;
	m_comboBoxShading->addItem("Flat");
	m_comboBoxShading->addItem("Smooth");
	m_comboBoxShading->addItem("Smooth+Texture");
	m_comboBoxShading->setCurrentIndex(m_shadingMode);
	...
	layout->addWidget(label[5],		5, 0);
	layout->addWidget(m_comboBoxShading,	5, 1);
	layout->addWidget(m_buttonStart,        6, 0);
	layout->addWidget(m_buttonStop,         6, 1);
...
HW3b::initShaderGeneric(int program, bool loadTexture)
{
	...
	if (loadTexture) {
		...
		DEBUG_ERR("initShaders before u_TextureEnabled ");
		m_uniform[program][DRAW_TEXTURE] = glGetUniformLocation(m_program[program].programId(), "u_TextureEnabled");
		if (m_uniform[program][DRAW_TEXTURE] < 0) {
			qDebug() << "Failed to get the storage location of u_TextureEnabled, program: " << program;
			exit(-1);
		}
	}
	...
	if (loadTexture) {
  	...
  	glUniform1i(m_uniform[program][SAMPLER], 0);
		glUniform1i(m_uniform[program][DRAW_TEXTURE], 0);
	}
...
void HW3b::getFaceNorms()
{
    ...
    //float geom0[3], geom1[3], geom2[3], geom3[3];
		face_norms.clear();
		vec3 v0, v1, v2, norm0, norm1;
		vec3 geom0, geom1, geom2, geom3;
    for(int i=0; i < m_grid-1; i++) {
        for(int j = 0; j < m_grid-1; j++) {
            // get vectors from geometry points
						geom0 = m_vertices[i][j];
						geom1 = m_vertices[i][j+1];
						geom2 = m_vertices[i+1][j];
						geom3 = m_vertices[i+1][j+1];

						v0 = geom1 - geom0;
						v1 = geom1 - geom2;
						v2 = geom1 - geom3;

						//qDebug() << QVector3D::crossProduct(v0, v1).normalized() << "\n";
						face_norms.push_back(QVector3D::crossProduct(v0, v1).normalized());
						face_norms.push_back(QVector3D::crossProduct(v1, v2).normalized());
        }
    }
}
...
void HW3b::uploadNormals()
{
		glBindBuffer(GL_ARRAY_BUFFER, m_normalsBuffer);
		glBufferData(GL_ARRAY_BUFFER, 2*m_numPoints*sizeof(vec3), &face_norms[0], GL_STATIC_DRAW);
